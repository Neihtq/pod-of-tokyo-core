...
class TestKubeDao(unittest.TestCase):
    ...
    def setUp(self, mock_core_v1_api, mock_load_kube_config, mock_subprocess_run):
        ...
        with patch(
            "controller_service.kube.kube_dao.client.CoreV1Api"
        ) as mock_client_core_v1_api:
    ...
    def test_list_all_namespaces(self):
        ...
        with patch(
            "controller_service.kube.kube_dao.MONSTER_NAMES_SET",
            new={"Gigazaur", "Meka Dragon"},
        ):
    ...
    def test_delete_all_namespaces(self):
        self.kube_dao.list_all_namespaces = MagicMock(
            return_value=["outside", "tokyo-city"]
        )
    ...
    def test_create_pod(self):
        ...
        node_port = self.kube_dao.create_pod(
            pod_name, namespace, player_id, service_port
        )
    ...
    def test_delete_pod(self):
        ...
        self.mock_core_v1_api.delete_namespaced_pod.assert_called_once_with(
            name=pod_name, namespace=namespace
        )
        self.mock_core_v1_api.delete_namespaced_service.assert_called_once_with(
    ...
    def test_move_pod(self):
        ...
        self.kube_dao.wait_for_pod_deletion.assert_called_once_with(
            pod_name, from_namespace
        )
        self.mock_core_v1_api.create_namespaced_pod.assert_called_once()
        self.kube_dao.expose_pod_port.assert_called_once_with(
            pod_name, target_namespace, service_port
        )
        self.kube_dao.wait_for_pod_ready.assert_called_once_with(
    ...
    def test_wait_for_pod_deletion_timeout(self):
        ...
        self.assertEqual(self.mock_core_v1_api.read_namespaced_pod.call_count, 1)
    ...
    def test_wait_for_pod_ready_success(self):
        ...
        mock_pod_ready = MagicMock(
            status=MagicMock(
                phase="Running", conditions=[MagicMock(type="Ready", status="True")]
            )
        )
        ...
        self.kube_dao.wait_for_pod_ready(pod_name, namespace, timeout=4)
        self.assertEqual(self.mock_core_v1_api.read_namespaced_pod.call_count, 2)
