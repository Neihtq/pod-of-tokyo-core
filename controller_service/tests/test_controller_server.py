...
class TestControllerServer(unittest.TestCase):
    ...
    def test_init_game(self):
        ...
        self.mock_kube_dao_instance.create_pod.side_effect = [
            30000,
            30001,
        ]
        self.mock_kube_dao_instance.list_all_namespaces.return_value = [
            "outside",
            "tokyo-city",
            "tokyo-bay",
        ]
    ...
    def test_destroy_tokyo_bay(self, mock_update_monster_location):
        ...
        self.server.players_by_id = {
            "player1": ("Gigazaur", "http://127.0.0.1:30000", 30000)
        }
        ...
        self.mock_kube_dao_instance.move_pod.assert_called_once_with(
            pod_name="Gigazaur",
            from_namespace="tokyo-bay",
            target_namespace="outside",
            service_port=30000,
        )
        mock_update_monster_location.assert_called_once_with(
            url="http://127.0.0.1:30000", location="outside"
        )
        self.mock_kube_dao_instance.delete_namespace.assert_called_once_with(
            "tokyo-bay"
        )
    ...
    def test_destroy_pod(self):
        ...
        response = self.app.post(
            "/destroyPod", json={"playerId": "player1", "location": "outside"}
        )
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.mock_kube_dao_instance.delete_pod.assert_called_once_with(
            "Gigazaur", "outside"
        )
        self.assertEqual(data["status"], "success")
