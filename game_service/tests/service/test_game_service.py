...
from pod_of_tokyo_commons.model import Location, PodStatus, MessageType
from pod_of_tokyo_commons.constants import TOKYO_CITY_KEY, TOKYO_BAY_KEY, OUTSIDE_KEY
...
class TestGameService(unittest.TestCase):
    def setUp(self):
        ...
        with patch("game_service.service.game_service.ControllerClient") as MockController:
    ...
    def test_start_game(self):
        ...
        # Mock wait_for_game_ready to avoid sleep loop
    ...
    def test_start_turn_outside_enters_tokyo_if_empty(self):
        ...
        # Mock controller to return empty Tokyo
        ...
        # Verify
        self.mock_controller.relocate.assert_called_with(player_id, OUTSIDE_KEY, TOKYO_CITY_KEY)
    ...
    def test_heal_player_outside(self):
        mock_pod = MagicMock()
        mock_pod.name = "P1"
        mock_pod.get_state.return_value = (5, 0, 0, OUTSIDE_KEY) # 5 health
    ...
    def test_heal_player_in_tokyo_fails(self):
        ...
        self.game_service.notifier.notify_all.assert_called_with("P1 is in Tokyo! Healing in Tokyo is not possible!")
    ...
    def test_reroll_dices(self):
        ...
        with patch("game_service.service.game_service.roll_dices") as mock_roll:
            ...
            self.game_service.notifier.call_and_wait.side_effect = [
                ...
                {"dices": ["1", "1"]}, 
                {"dices": ["P", "P"]},
                {"dices": ["E", "E"]} # Add one more for safety/logic
            ]
            
            final_dices = self.game_service.reroll_dices(mock_pod)
            
            # Logic in reroll_dices:
            # Loop 1: throws 6, keeps 2 ("1","1"), num_throws becomes 4
            # Loop 2: throws 4, keeps 2 ("P","P"), num_throws becomes 2
            # Loop 3: throws 2... wait, loop condition is throw_count < max_num_throws (3)
            # So it does 3 throws.
            # After loop, if len(dices_to_keep) < 6, it adds remaining from last throw.
            # Last throw was ["E", "E"].
            # So final should be ["1", "1", "P", "P", "E", "E"]
    ...
    def test_slap_damages_others(self):
        ...
        target_pod.name = "P2"
        target_pod.get_state.return_value = (10, 0, 0, OUTSIDE_KEY) # Outside
    ...
    def test_slap_kills_player(self):
        ...
        target_pod.player_id = "p2"
        target_pod.name = "P2"
        target_pod.get_state.return_value = (2, 0, 0, OUTSIDE_KEY) # 2 health
    ...
    def test_slap_yield_tokyo(self):
        ...
        self.mock_controller.relocate.assert_called_with("p2", TOKYO_CITY_KEY, OUTSIDE_KEY)
    ...
    def test_decide_starter(self):
        ...
        with patch("game_service.service.game_service.roll_dices") as mock_roll:
            mock_roll.side_effect = [
                [DiceSymbols.FIST.value], # p1
                [DiceSymbols.FIST.value, DiceSymbols.FIST.value] # p2
            ]
            
            self.game_service.manager.get_player_index.return_value = 1 # p2 is index 1
    ...
    def test_destroy_tokyo_bay(self):
        ...
        self.game_service.notifier.notify_all.assert_any_call("Tokyo Bay has been flooded!")
    ...
    def test_fill_empty_space_bay(self):
        ...
        self.mock_controller.relocate.assert_called_with("p1", OUTSIDE_KEY, TOKYO_BAY_KEY)
    ...
    def test_start_turn_fill_empty_space(self):
        ...
        # Mock fill_empty_space to return a location
        with patch.object(self.game_service, 'fill_empty_space') as mock_fill:
            ...
            # Should not call fill_empty_space again at the end
    ...
    def test_reroll_dices_keeps_remaining(self):
        ...
        # Roll 6 dice, keep 0 explicitly
        # Should auto-keep all 6
        with patch("game_service.service.game_service.roll_dices") as mock_roll:
    ...
    def test_decide_starter_tie_break(self):
        ...
        with patch("game_service.service.game_service.roll_dices") as mock_roll:
            mock_roll.side_effect = [
                [DiceSymbols.FIST.value], # p1: 1 fist
                [DiceSymbols.FIST.value], # p2: 1 fist (Tie)
                [DiceSymbols.FIST.value], # p1: 1 fist
                [DiceSymbols.FIST.value, DiceSymbols.FIST.value] # p2: 2 fists (Win)
            ]
