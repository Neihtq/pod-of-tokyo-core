...
class TestControllerClient(unittest.TestCase):
    ...
    def test_init_game(self, mock_post):
        ...
        mock_post.assert_called_once_with(self.base_url, "initGame", {"players": players_data})
    ...
    def test_get_pod_url(self, mock_post):
        ...
        mock_post.assert_called_once_with(self.base_url, "getPodUrl", {"playerId": player_id})
    ...
    def test_destroy_pod(self, mock_post):
        ...
        mock_post.assert_called_once_with(self.base_url, "destroyPod", {"player_id": player_id, "location": location})
        self.assertEqual(response, {"status": "pod destroyed"})

    @patch('game_service.utils.http_utils.post')
    def test_get_node_state(self, mock_post):
        mock_post.return_value = {"tokyoCity": "player1", "tokyoBay": None, "outside": []}
        response = self.client.get_node_state()
        mock_post.assert_called_once_with(self.base_url, "getNodeState")
        self.assertEqual(response, {"tokyoCity": "player1", "tokyoBay": None, "outside": []})
